...
from keras.layers import Dense, Conv2D, AveragePooling2D, Input, Lambda, GlobalAveragePooling2D, GlobalAveragePooling1D, GlobalAveragePooling3D
...
def modify_and_dump_data():
    ...
    TRAIN_DATA_LABELS = to_categorical(np.concatenate(np.load("/home/kiriteegak/Desktop/github-general/"
                                                              ...
                                                              "train-labels.npy")[:2]), num_classes=46)
...
def cnn_architecture(X=None, slice_tensors=None, train_data_labels=None):
    ...
    custom_pooling = Input(shape=(None, 477, 1), dtype='float32', name='custom_pooling')
    ...
    print(pool2)
    # pool3 = Lambda(lambda a: tf.squeeze(a, axis=[2]))(pool2)
    # print(pool3)
    global_avg_pooling = GlobalAveragePooling2D()(pool2)
    transposed_pooling = Lambda(lambda a: K.transpose(a))(global_avg_pooling)
    print(transposed_pooling)
    exit()
    dense1 = Dense(units=100, init='normal', activation='relu')(transposed_pooling)
    dense2 = Dense(units=46, init='normal', activation='softmax')(dense1)
    keras_tensor_conv_2 = Lambda(lambda a: tf.squeeze(a, axis=[0]))(dense2)

    model = Model(inputs=[main_input, custom_pooling], outputs=keras_tensor_conv_2)
    model.compile(optimizer=sgd_, loss=categorical_crossentropy, metrics=['accuracy'])
    model.summary(line_length=200)

    # model.fit(x=[X, slice_tensors],
    #           y=train_data_labels,
    #           batch_size=10,
    #           shuffle=True,
    #           epochs=200)
