...
class PreProcessing(object):
    def __init__(self, folder_name, file_ending, rotate_=True):
...
def cnn_architecture():
    ...
    #                      rotate_=True).chain_pre_processes()
    #
    ...
    model.add(Dropout(0.2))
    model.add(Activation('relu'))
    model.add(Conv2D(filters=192, kernel_size=(1, 1), border_mode='same'))
    model.add(Dropout(0.2))
    model.add(Activation('relu'))
    model.add(Conv2D(filters=10, kernel_size=(1, 1), border_mode='same'))
    model.add(Dropout(0.2))
    model.add(Activation('relu'))
    model.add(AveragePooling2D(pool_size=(6, 6)))

    model.add(Flatten())
    model.add(Dense(units=10, init='normal'))
    ...
    model.save("models/cifar_10_1000_1000_10_1.h5py")


cnn_architecture()
# model = load_model("models/cifar_10_1000_1000_10_1.h5py")
# x_test, y_test = PreProcessing(folder_name="cifar-10-batches-py",
#                                file_ending="test_batch",
#                                rotate_=False).chain_pre_processes()
